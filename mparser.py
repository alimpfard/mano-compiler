...
def parse_func_head(tokenlist, func):
  ...
  try:
    ...
    if key == 'EXTERN':
        ...
        if len(line):
          raise ManoParserError('Garbage found after extern header.')
        return name
...
def parse_code(tokenlist, func):
  ...
  while tokenlist.peek() != ['END']:
    try:
      ...
      if line[0] == 'GOTO':
      ...
      elif line[0] == 'CLEARCARRY':
          code = SpecialLine('CLEARCARRY', [None], label, condition)
      elif line[0] == 'ROTRIGHT' or line[0] == 'ROTLEFT':
          code = SpecialLine(line[0], [line[1]], label, condition)
      elif '=' in line:
        target = line.pop(0)
        ...
        raise ManoParserError('Could not parse statement ' + str(line))
    except IndexError:
      raise ManoParserError('Error parsing statement.' + str(line))
...
def parse_expression(tokens, func):
  if len(tokens) == 1:
    if tokens[0] == 'CARRYVAL' or isValidIdentifier(tokens[0]):
    ...
    raise ManoParserError('Could not parse expression ' + str(tokens))
