...
class SpecialLine(CodeLine):
    def __init__(self, func, args, label=None, condition=None):
        self.func= func
        self.arg = args[0]
        CodeLine.__init__(self, label, condition)

    def __repr__(self):
        return '%s Special: %s (%s)' % (CodeLine.__repr__(self), self.func, self.arg)

class GetFlagLine(CodeLine):
    def __init__(self, flag, label=None, condition=None):
        self.name = flag
        CodeLine.__init__(self, label, condition)

    def __repr__(self):
        return '%s Flag %s' % (CodeLine.__repr__(self), self.flag)
